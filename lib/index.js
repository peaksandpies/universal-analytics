...
module.exports.koa = function (tid, options) {
	...
	return function *(next) {
		...
		this.req.visitor = module.exports.createFromSession(this.session);

		if (this.req.visitor) return yield next;

		var cid;
		if (this.req.cookies && this.req.cookies[cookieName]) {
			var gaSplit = this.req.cookies[cookieName].split('.');
			cid = gaSplit[2] + "." + gaSplit[3];
		}

		this.req.visitor = init(tid, cid, options);

		if (this.session) {
			this.session.cid = this.req.visitor.cid;
		}
